	...
	<div>
		...
		<div class="search-input">
            ...
            <!--
                ...
                按上下键可以选择列表条目
            -->
			<span class="search-reset" @click="clearInput()">&times;</span><!--删除输入框-->
			<button class="search-btn" @click="search()">搜搜看</button>
			<div class="search-select">
				<transition-group name="itemfade" tag="ul" mode="out-in" v-cloak>
				<!--tag='ul'表示用ul包裹v-for出来的li-->
...
export default{
    ...
    methods:{
    	...
    	selectUp:function(){
    		...
    		//从列表最后一个往上走
    		if (this.now == -1) {
    			this.keyword = this.myData.length - 1;
    		}
    		this.keyword = this.myData[this.now];
    	},
    	search:function(){
...
input::-ms-clear {
    display: none;
    color: green;
    /*::-ms-clearCSS 伪元素表示在文本的边缘处的按钮（“清除按钮”）*/
}
